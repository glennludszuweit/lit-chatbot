...
class ChatController {
     ...
     */
    scrollToBottom() {
        const lastMessage = this.shadowRootDoc.querySelector('#chat-messages').lastChild;
        if (lastMessage) {
     ...
     */
    showTypingIndicator() {
        const chatMessageWindow = this.shadowRootDoc.querySelector('#chat-messages');
     ...
     */
    hideTypingIndicator() {
        const typingIndicator = this.shadowRootDoc.querySelector('#typing-indicator');
        if (typingIndicator) {
            const chatMessageWindow = this.shadowRootDoc.querySelector('#chat-messages');
    ...
    createElement(type, params) {
        ...
        if (params) {
            ...
            if (params.onsubmit && typeof params.onsubmit === 'string')
                ...
                elem.onsubmit = params.onsubmit;
            if (params.oninput && typeof params.oninput === 'string')
                elem.setAttribute('oninput', params.oninput);
            else if (params.oninput)
                elem.oninput = params.oninput;
            if (params.rel)
                elem.setAttribute('rel', params.rel);
            if (params.href)
                elem.setAttribute('href', params.href);
            if (params.src)
        ...
        for (let b = 0; b < buttons.length; b++) {
            ...
            if (buttons[b] && buttons[b]['link']) {
                const buttonElem = this.createElement('button', {
                    ...
                    onclick: 'chatController.openButtonLink(' +
                ...
                const buttonElem = this.createElement('button', {
                    className: 'convButton',
                    innerText: buttons[b].name,
                    onclick: 'chatController.sendQuickReply(this, ' +
     ...
     */
    addCarousel(messageContainer, carousel, displayUnselectedOptions, hideButtons) {
        // do not create already used carousel slides if they shouldn't been displayed
        if (hideButtons && !displayUnselectedOptions)
            return;
        const carouselContainer = this.createElement('div', {
            className: 'carousel-container',
        });
    ...
    addFeedbackTiles(messageContainer, feedbackTiles) {
        ...
        for (let t = 0; t < feedbackTiles.length; t++) {
            const tileElem = this.createElement('button', {
                className: 'convButton',
                innerHTML: '<img style="float:left"  src="' +
        ...
        setTimeout(function () {
            ...
            if (chatController.datePicker.length > 1) {
                ...
                chatController.datePicker.on('change', function () {
                    var data = document.getElementsByClassName(chatController.dateClass)[0].value;
                    // chatController.datePicker.close();
                    document.getElementById('user-input').value = data;
                    document.getElementById('user-input').focus();
                });
                datepickerElemContainer.appendChild(document.getElementById('tail-datetime-' + chatController.dateCounter));
            ...
            if (fieldData.type === 'dropdown' && fieldData.dropdownValues) {
                ...
                for (let i = 0; i < fieldData.dropdownValues.length; i++) {
                    const option = this.createElement('div', {
                        ...
                        'data-value': fieldData.dropdownValues[i].value ||
                            fieldData.dropdownValues[i][fieldData.variableValue],
                        innerText: fieldData.dropdownValues[i].value ||
                        ...
                        onclick: (e) => {
                            ...
                            chatController.updateFormValue(fieldData.title, e.target.dataset.value, e.target.parentNode.parentNode.parentNode);
                        },
                    });
                ...
                for (let i = 0; i < fieldData.radioValues.length; i++) {
                    const button = this.createElement('div', {
                        ...
                        onclick: (e) => {
                            ...
                            chatController.updateFormValue(fieldData.title, values, e.target.parentNode.parentNode);
                        },
                    });
            ...
            if (fieldData.type === 'checkbox') {
                ...
                const checkbox = this.createElement('input', {
                    ...
                    onchange: (e) => {
                        ...
                        chatController.updateFormValue(fieldData.title, value, e.target.parentNode.parentNode.parentNode);
                    },
                });
            ...
            if (buttons[b].name) {
                clickAction = (e) => {
                    ...
                    const invalidFields = form.data
                        ...
                        this.activeForm[item.title] &&
                        new RegExp(item.regexpPattern, item.regexpFlags.join('')).test(this.activeForm[item.title]) === false)
                        .map((item) => item.title);
                    ...
                    if (errorFields.length === 0 && invalidFields.length === 0) {
                        this.sendFormReply(e.target.parentNode.parentNode, buttons[b].name, form.id);
                    }
                };
            }
            else {
    ...
    addCustomForm(messageContainer, data) {
        ...
        const formConfirmBtn = this.createElement('button', {
            ...
            onclick: () => {
                ...
                socketController.sendUserMessageToServer(JSON.stringify(jsonObj), false);
            },
        });
    ...
    async displayMessageOnChat(isAgentMsg, content, hideButtons) {
        ...
        if (isAgentMsg) {
            ...
            try {
                if (messages && messages.length >= 0) {
                    ...
                    messages.forEach(function (txt, idx, messages) {
                        var tokens = messages[idx].match(/\b(?:http:\/\/|https:\/\/)(\S+)\b/g); // extract all url's
                        if (tokens) {
                            tokens.forEach(function (token, cnt, tokens) {
                                ...
                                const messageBeforeUrl = messages[idx].substring(0, messages[idx].indexOf(token));
                                if (!messageBeforeUrl.endsWith('href="')) {
                                    messages[idx] = messages[idx].replace(token, '<a class="linkstyle" href="' +
                                        token +
                                        '" target="_blank" rel="noopener noreferrer">' +
                                        token +
                                        '</a>'); // replace each with anchor tag
                                }
                            });
                        }
                    });
            ...
            content.responseConfig = this.sanitizeResponseConfig(content.responseConfig || {});
            const customForm = content.responseConfig &&
            ...
            if (content.responseConfig.feedbackTiles) {
                this.addFeedbackTiles(messageContainer, content.responseConfig.feedbackTiles);
            }
            // add carousel
            if (content.responseConfig.carousel) {
                document.getElementById('user-input').disabled = false;
                document.getElementById('chat-input-send').disabled = false;
                this.addCarousel(messageContainer, content.responseConfig.carousel, content.responseConfig.displayUnselectedOptions, hideButtons);
            }
            // add datepicker
            ...
            if (content.responseConfig.form && !hideButtons) {
                document.getElementById('user-input').disabled = false;
                document.getElementById('chat-input-send').disabled = false;
                this.addForm(messageContainer, content.responseConfig.form, content.responseConfig.buttons);
            }
            // add custom form
            ...
            // add buttons
            if (content.responseConfig.buttons &&
    ...
    maximizeChat() {
        ...
        if (chatState === 'maximize') {
            ...
            this.shadowRootDoc.querySelector('.user-interaction-bar-maximize').className = 'user-interaction-bar';
        }
        else {
            ...
            this.shadowRootDoc.querySelector('.user-interaction-bar-maximize').style.width = '97%';
            this.shadowRootDoc.querySelector('.user-interaction-bar-maximize').style.margin = '10px auto';
            this.shadowRootDoc.querySelector('.user-interaction-bar-maximize').style.height = '50px';
        }
    }
    /**
    ...
    initChatWindow() {
        ...
        const reloadBtn = this.createElement('div', {
            ...
            id: 'reload-btn',
            onclick: 'socketController.resetConversation({ resetButtonClicked: true, keepChatMsgs: false })',
        });
    ...
    getLastResponse() {
        const messages = this.shadowRootDoc.querySelector('#chat-messages').children;
        let manageResponses = [];
        ...
        function RemoveTimeStamp() {
            ...
            const responseSeconds = responseTime.replace(/(vor wenigen Sekunden)/gi, ' ');
